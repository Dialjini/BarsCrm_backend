...
def index():
    ...
    if 'username' in session:
        return render_template('index.html', last_update=4424)
    else:
...
def updateRegions():
    js = request.args['json'].encode('utf-8').decode('utf-8')
    with open(os.path.abspath(os.path.dirname(__file__) + '/static/js/json/regions.json'), 'wb') as file:
        json.dump(js, file)
