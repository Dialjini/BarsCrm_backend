    ...
    `}let list_items_acc,list_stock_acc;function checkStocks(a){let b=a.name.split("_")[1],c=[];for(let b of $("#group tr")){let a=$(b)[0].children[0].children[0].value,d=$(b)[0].children[1].children[0].value;c.push({date:a,sum:d})}0==c.length&&c.push({date:"",sum:""}),$.ajax({url:"/addAccountPaymentHistory",data:{account_id:+b,account_payment_history:JSON.stringify(c)},type:"GET",dataType:"html",success:function(){function b(){let b="";for(let d=0;d<c.length;d++)for(let f=0;f<e.length;f++)if(c[d].id==e[f].stock){b+=`<button class="selectStock" name="${a.name}" id="${a.id}" onclick="arrangeDelivery(this)" data-items="${e[f].items}" data-stock="${c[d].Name}">${c[d].Name}</button>`;break}return b}list_stock_acc=$("#stock_items_list").attr("data-stock").split(","),list_items_acc=$("#stock_items_list").attr("data-items").split(",");let c,d,e=[];$.ajax({url:"/getStocks",type:"GET",async:!1,dataType:"html",success:function(a){c=JSON.parse(a)}}),$.ajax({url:"/getAllItems",type:"GET",async:!1,dataType:"html",success:function(a){d=JSON.parse(a)}});for(let a=0;a<c.length;a++)for(let b=0;b<list_stock_acc.length;b++)list_stock_acc[b]==c[a].id&&e.push({stock:c[a].id,items:[]});for(let a=0;a<e.length;a++)for(let b=0;b<d.length;b++)e[a].stock==d[b].Stock_id&&e[a].items.push(d[b].Item_id);if(1<list_stock_acc.length)$(".page").prepend($("<div>",{class:"background"})),$(".page").prepend($("<div>",{class:"modal_select",append:`
        ...
        <div class="row_card column">
            ...
            <div class="list" id="list_contract">
                ${function(){let b,c="";$.ajax({url:"/getDocs",type:"GET",async:!1,dataType:"html",success:function(a){b=JSON.parse(a)}});let d=[{category:"client",name:"\u0414\u043E\u0433\u043E\u0432\u043E\u0440 \u043F\u043E\u0441\u0442\u0430\u0432\u043A\u0438 \u043E\u0442"},{category:"carrier",name:"\u0414\u043E\u0433\u043E\u0432\u043E\u0440 \u043E\u0431 \u043E\u043A\u0430\u0437\u0430\u043D\u0438\u0438 \u0443\u0441\u043B\u0443\u0433 \u043F\u0435\u0440\u0435\u0432\u043E\u0437\u043A\u0438 \u0433\u0440\u0443\u0437\u043E\u0432 \u043E\u0442"}];a.name.includes("new")&&(a.name=a.name.replace(/new/g,saveTableAndCard[1][1].length+1));for(let e=0;e<b.length;e++)for(let f=0;f<d.length;f++)b[e].Owner_type==a.id&&a.id==d[f].category&&b[e].Owner_id==a.name.split("_")[1]&&(c+=`
                    <div style="background-color: #E8E8E8" class="contract flex" name="${d[f].name} ${b[e].Prefix}" id="${b[e].id}" onclick="downloadOldDocument(this)">
                        <span style="text-align: center">${d[f].name} ${b[e].Prefix}</span>
                    </div>
                    `);return c+=`
        <div class="contract flex" name="${a.name}" onclick="downloadDocument(this)">
        ...
        <div class="next">
            ${function(){if(a.name.includes("client"))return`
                <button class="btn" style="margin-right: 10px" id="${a.id}" onclick="comeBack(this.id)">Назад</button> 
                <button class="btn btn-main" id="${a.id}" onclick="invoiceCard(this)">Выставить счёт</button> 
            `;if(a.name.includes("carrier")){let b=$(a).attr("name").split("_");return`
                <button class="btn" style="margin-right: 10px" id="${a.id}" onclick="comeBack(this.id)">Назад</button> 
                <button class="btn btn-main" name="${b[0]}_close_card_${b[1]}_contract" onclick="closeCardMenu(this.name)">Закрыть</button> 
            `}}()}
        </div>
    `}function downloadOldDocument(a){const b=document.createElement("a");b.href=`/downloadOldDoc?id=${a.id}`,b.download=$(a).attr("name")+".docx",b.click()}function downloadDocument(a){let b=$(a).attr("name").split("_"),c=$("#select_cusmoter").val();"client"==b[0]?$.ajax({url:"/getClients",type:"GET",dataType:"html",success:function(a){a=JSON.parse(a);for(let d=0;d<a.length;d++)if(a[d].id==b[1]){let e="\u041E\u041E\u041E"==c?"Dogovor_na_tovari_ooo":"Dogovor_na_tovari_ip";const f=document.createElement("a");f.href=`/downloadDoc?category=${b[0]}&name=${e}&card_id=${b[1]}&address=${a[d].Adress}&delivery=no`,f.download="\u041E\u041E\u041E"==c?"\u0414\u043E\u0433\u043E\u0432\u043E\u0440 \u043F\u043E\u0441\u0442\u0430\u0432\u043A\u0438 \u041E\u041E\u041E.docx":"\u0414\u043E\u0433\u043E\u0432\u043E\u0440 \u043F\u043E\u0441\u0442\u0430\u0432\u043A\u0438 \u0418\u041F.docx",f.click()}}}):"carrier"==b[0]&&$.ajax({url:"/getCarriers",type:"GET",dataType:"html",success:function(a){a=JSON.parse(a);for(let d=0;d<a.length;d++)if(a[d].id==b[1]){let e="\u041E\u041E\u041E"==c?"DogovorNaDostavkuOOO":"DogovorNaDostavkuIP";const f=document.createElement("a");f.href=`/downloadDoc?category=${b[0]}&name=${e}&card_id=${b[1]}&address=${a[d].Address}&delivery=no`,f.download="\u041E\u041E\u041E"==c?"\u0414\u043E\u0433\u043E\u0432\u043E\u0440 \u043D\u0430 \u0434\u043E\u0441\u0442\u0430\u0432\u043A\u0443 \u041E\u041E\u041E.docx":"\u0414\u043E\u0433\u043E\u0432\u043E\u0440 \u043D\u0430 \u0434\u043E\u0441\u0442\u0430\u0432\u043A\u0443 \u0418\u041F.docx",f.click()}}})}function invoicingContentCard(a,b){function c(a,b,c){let d=$("<div>",{class:"row_card",append:$("<div>",{class:"info_block full "+c,append:$("<table>",{class:"account_table",id:b,html:a[0](),append:a[1]()})})});return d}return function(){let a=$("<div>",{class:"row_card",append:$("<div>",{class:"costs gray",html:` <div class="costs_element">
                        ...
                        </div>`})});return a}().add(c([function(){return`<tr>
